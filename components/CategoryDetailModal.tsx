...
// FIX: Added imports for PlusCircleIcon and Trash2Icon which were missing.
...
const CategoryDetailModal: React.FC<CategoryDetailModalProps> = ({ isOpen, onClose, category, categoryDetail, planStatus, onUpdate }) => {
    ...
    const handleSubItemChange = (index: number, field: 'name' | 'value' | 'notes', value: string | number) => {
        ...
        // FIX: Simplified the logic to directly update the property without confusing type checks.
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div
                className="bg-card rounded-2xl border border-border shadow-2xl w-full max-w-4xl p-6 flex flex-col animate-fade-in-up"
                onClick={e => e.stopPropagation()}
                style={{ maxHeight: '90vh' }}
            >
                <div className="flex items-center justify-between mb-6 flex-shrink-0">
                    <div>
                        <h2 className="text-xl font-bold text-foreground">Detalhes da Categoria: {CATEGORY_NAMES[category]}</h2>
                        <p className="text-sm text-muted-foreground">Adicione sub-itens, ajuste a sazonalidade e defina um responsável.</p>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 overflow-y-auto pr-2">
                    ...
                    <div className="space-y-4">
                            ...
                            {detail.subItems.map((item, index) => (
                                <div key={item.id} className="flex items-center gap-2 p-2 bg-background rounded-lg">
                                    <input
                                        ...
                                        className="flex-grow bg-transparent font-medium p-1 rounded-md focus:outline-none focus:bg-card"
                                        readOnly={isReadOnly}
                                    />
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            className="w-28 bg-transparent font-semibold p-1 pl-7 text-right rounded-md focus:outline-none focus:bg-card"
                                            readOnly={isReadOnly}
                                        />
                                    </div>
                                    {!isReadOnly && <button onClick={() => removeSubItem(index)} className="text-red-500 hover:text-red-700 p-1"><Trash2Icon className="w-4 h-4" /></button>}
                                </div>
                            ))}
                        </div>
                        {!isReadOnly && (
                            <div className="flex items-center gap-2">
                                <input
                                    ...
                                    className="flex-grow bg-background border border-border p-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
                                />
                        ...
                        <div className="flex justify-between items-center bg-card-alt p-3 rounded-lg mt-2">
                        ...
                        <input
                            ...
                            className="w-full bg-background border border-border p-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
                    ...
                    <div>
                        <h3 className="font-semibold text-foreground">Pesos Sazonais (%)</h3>
                        <p className="text-xs text-muted-foreground mb-2">Distribua o total anual pelos meses.</p>
                                ...
                                <div key={month} className="flex items-center justify-between">
                                    ...
                                    <div className="relative w-20">
                                        <input
                                            ...
                                            className="w-full bg-background p-1 pr-5 text-right rounded-md border border-border focus:outline-none focus:ring-2 focus:ring-primary"
                                            step="0.01"
                                            readOnly={isReadOnly}
                                        />
                                        <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-1.5 text-muted-foreground text-sm">%</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                         <div className={`flex justify-between items-center p-3 rounded-lg mt-4 font-bold ${Math.abs(totalWeights - 1) > 0.001 ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>

...
const PanelCard: React.FC<{ title: string, children: React.ReactNode, className?: string }> = ({ title, children, className }) => (
    <div className={`bg-card p-6 rounded-xl border border-border hover-zoom ${className}`}>
...
const NumericInput: React.FC<{
    ...
    return (
        <div>
            <label htmlFor={name} className="block text-sm font-medium text-muted-foreground mb-1.5">{label}</label>
            <div className="relative">
                <span className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-muted-foreground">R$</span>
                <input
                    type="text"
                    id={name}
                    name={name}
                    ...
                    onBlur={handleBlur}
                    className={`w-full rounded-lg border border-border bg-background px-3 py-2 text-right font-semibold text-foreground transition duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary pl-10 ${isMainInput ? 'text-lg' : 'text-base'}`}
                    placeholder="0,00"
                />
            </div>
        </div>
            ...
            <div className="flex items-center space-x-3 w-3/5">
                <div className="relative w-1/2">
                    <input
                        ...
                        className="w-full rounded-md border-border bg-background shadow-sm pr-7 pl-2 py-1 text-right font-semibold text-foreground transition duration-150 ease-in-out focus:bg-card focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary text-sm"
                        placeholder="0"
                        step="0.01"
                    />
                    <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2 text-muted-foreground text-sm">%</span>
                </div>
                <div className="w-1/2 h-8 flex items-center justify-end px-2 bg-background rounded-md">
                    <span className="text-foreground font-medium text-sm tabular-nums">{formatCurrency(calculatedValue)}</span>
                </div>
            </div>
        </div>
    );
...
const ResultMetric: React.FC<{ icon: React.ReactNode; label: string; value: string; isHighlighted?: boolean }> = ({ icon, label, value, isHighlighted }) => (
    <div className={`flex items-center space-x-3 p-3 rounded-lg border-l-4 ${isHighlighted ? 'bg-primary-soft border-primary' : 'bg-background border-border'}`}>
        <div className={`flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center p-1.5 ${isHighlighted ? 'bg-primary/20 text-primary' : 'bg-border text-muted-foreground'}`}>
        ...
        <div>
            ...
            <FormattedValue
                value={value}
                integerClassName={`font-bold tabular-nums tracking-tight ${isHighlighted ? 'text-primary text-xl' : 'text-foreground text-lg'}`}
                fractionClassName={`font-semibold tabular-nums ${isHighlighted ? 'text-primary/80' : 'text-muted-foreground'}`}
                symbolClassName={`font-semibold ${isHighlighted ? 'text-primary/80' : 'text-muted-foreground'}`}
            />
        </div>
    </div>
);
        ...
        <>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                    ...
                    <div className="space-y-6">
                        <PanelCard title="Simulação de Produto">
                            <div className="space-y-4">
                                <input
                                    ...
                                    className="w-full text-lg font-bold bg-background p-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:bg-card border border-transparent focus:border-primary text-foreground"
                                />
                                ...
                                <div className="grid grid-cols-2 gap-4 pt-2">
                                     <div className="bg-background p-3 rounded-lg text-center flex flex-col justify-center">
                                        <p className="text-sm text-muted-foreground font-medium">Markup Atual</p>
                                        <p className={`text-xl font-bold ${results.currentMarkup < 2.5 ? 'text-accent-red' : (results.currentMarkup >= 3 ? 'text-accent-green' : 'text-accent-orange')}`}>{formatDecimal(results.currentMarkup)}x</p>
                                    </div>
                                    <button 
                                        onClick={applyRecommendedPrice}
                                        className="bg-primary-soft p-3 rounded-lg text-center relative group cursor-pointer hover:bg-primary-soft/80 transition-colors flex flex-col items-center justify-center"
                                    >
                                        <p className="text-sm text-primary-soft-fg font-medium">Aplicar Recomendado (3x)</p>
                                        <p className="text-xl font-bold text-primary">{formatCurrency(results.recommendedPrice)}</p>
                                    </button>
                                </div>
                            </div>
                        </PanelCard>
                        ...
                        <PanelCard title="Custos Variáveis de Venda (%)">
                             ...
                             <div className="!mt-4 flex justify-between items-center bg-background p-3 rounded-lg">
                                <span className="font-semibold text-foreground">Total Custos Variáveis</span>
                                <span className="font-bold text-lg text-foreground">{formatCurrency(results.fixedCostsValue + results.marketingValue)}</span>
                             </div>
                        </PanelCard>
                    ...
                    <div className="bg-card p-6 rounded-xl border border-border hover-zoom flex flex-col h-full">
                        <h2 className="text-xl font-bold text-foreground mb-6">Cenários de Rentabilidade</h2>
                        <div className="overflow-x-auto flex-1">
                            <table className="w-full border-collapse min-w-[600px]">
                                <thead>
                                    <tr className="border-b border-border">
                                        <th className="p-3 text-left text-xs font-semibold uppercase text-muted-foreground tracking-wider bg-background sticky top-0">Markup</th>
                                        <th className="p-3 text-right text-xs font-semibold uppercase text-muted-foreground tracking-wider bg-background sticky top-0">Preço Venda</th>
                                        <th className="p-3 text-right text-xs font-semibold uppercase text-muted-foreground tracking-wider bg-background sticky top-0">CPA Máx.</th>
                                        <th className="p-3 text-right text-xs font-semibold uppercase text-muted-foreground tracking-wider bg-background sticky top-0">Lucro Líquido</th>
                                        <th className="p-3 text-right text-xs font-semibold uppercase text-muted-foreground tracking-wider bg-background sticky top-0">Margem %</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {markupTableData.map((row) => {
                                        ...
                                        return (
                                            <tr key={row.markup} className={`text-right border-b border-border last:border-b-0 transition-colors ${isHighlighted ? 'bg-primary-soft' : 'hover:bg-card-alt'}`}>
                                                <td className={`p-3 text-left font-semibold ${isHighlighted ? 'text-primary-soft-fg' : 'text-foreground'}`}>{formatDecimal(row.markup)}x</td>
                                                <td className={`p-3 font-semibold tabular-nums ${isHighlighted ? 'text-foreground' : 'text-foreground'}`}>{formatCurrency(row.finalPrice)}</td>
                                                <td className="p-3 tabular-nums text-muted-foreground font-medium">{formatCurrency(row.maxCpa)}</td>
                                                <td className={`p-3 font-bold tabular-nums ${row.profit < 0 ? 'text-accent-red' : 'text-accent-green'}`}>{formatCurrency(row.profit)}</td>
                                                <td className={`p-3 font-semibold tabular-nums ${row.profitPercentage < 10 ? 'text-accent-red' : (row.profitPercentage > 20 ? 'text-accent-green' : 'text-accent-orange')}`}>{formatPercentage(row.profitPercentage)}</td>
                                            </tr>
                                        );
                                    })}
                                </tbody>
            ...
            {activeTab === 'costs' && (
                 <ProductCosts
                    ...
                    // FIX: Changed onViewDetails to onViewProductDetails to match the prop passed to PricingCalculator.

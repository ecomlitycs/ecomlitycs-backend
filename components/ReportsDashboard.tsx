...
const Badge: React.FC<{ children: React.ReactNode, variant?: 'default' | 'outline' | 'success' | 'warning' | 'danger', className?: string }> = ({ children, variant = 'default', className = '' }) => {
    ...
    const variants = {
        ...
        warning: "bg-accent-orange/10 text-accent-orange hover:bg-accent-orange/20",
...
const Card: React.FC<{ children: React.ReactNode, className?: string }> = ({ children, className = '' }) => (
    <div className={`rounded-xl border border-border bg-card text-foreground shadow-sm ${className}`}>{children}</div>
)
    ...
    return (
        <div className="h-[240px] w-full flex items-end justify-between space-x-2 pt-8 pb-6">
            {data.map((item, i) => {
                ...
                if (item.type === 'subtotal') colorClass = 'bg-muted-foreground/30';
                ...
                return (
                    <div key={i} className="flex-1 flex flex-col items-center relative group h-full justify-end">
                         <div className="absolute -top-6 text-[10px] font-semibold text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
                        ...
                        <span className="text-[10px] text-muted-foreground mt-2 truncate w-full text-center" title={item.label}>{item.label}</span>
                    </div>
                )
            })}
        </div>
...
const BurnDownChart: React.FC<{ expected: {date:string, value:number}[], actual: {date:string, value:number}[] }> = ({ expected, actual }) => {
    ...
    return (
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible" preserveAspectRatio="none">
             {/* Grid lines */}
             <line x1="0" y1={height} x2={width} y2={height} stroke="currentColor" className="text-border" strokeWidth="1" opacity="0.5" />
             <line x1="0" y1="0" x2="0" y2={height} stroke="currentColor" className="text-border" strokeWidth="1" opacity="0.5" />
        ...
        <div className="flex flex-col space-y-4 pt-4">
            {data.map((item, i) => (
                 <div key={i} className="flex items-center group">
                    <div className="w-32 text-sm text-muted-foreground font-medium truncate pr-4 text-right" title={item.label}>{item.label}</div>
                    <div className="flex-1 h-8 bg-card-alt rounded-r-md overflow-hidden relative">
                         <div 
                            className="h-full bg-primary/80 absolute left-0 top-0 rounded-r-md flex items-center justify-end px-3 transition-all duration-500 group-hover:bg-primary" 
    ...
    return (
        <div className="relative flex-shrink-0" style={{width: size, height: size}}>
            <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="-rotate-90">
                <circle className="text-card-alt/50" stroke="currentColor" strokeWidth={strokeWidth} fill="transparent" r={radius} cx={size / 2} cy={size / 2} />
             ...
             <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                <Card className="col-span-2 shadow-sm hover-zoom">
                    ...
                    <CardContent>
                        ...
                        <div className="text-sm text-muted-foreground font-medium text-center">{ui.footer}</div>
                    </CardContent>
                </Card>
                ...
                <Card className="shadow-sm hover-zoom flex flex-col">
                    <CardHeader className="pb-2">
                        <CardTitle>Burn-down da Meta</CardTitle>
                         <div className="flex items-center gap-2 text-sm mt-1">
                            ...
                            <span className="text-muted-foreground">Progresso: <strong className="text-foreground">{Math.round(goal.progress_pct)}%</strong></span>
                        </div>
                    </CardHeader>
                    <CardContent className="flex-1 min-h-[200px] pt-6 flex flex-col justify-between">
                        <div className="h-40">
                         ...
                         <div className="mt-4 text-sm flex justify-between text-muted-foreground pt-4 border-t border-border">
                            <span>Meta: <strong className="text-foreground">{formatCurrency(goal.value)}</strong></span>
                            <span>Faltam: <strong className="text-foreground">{formatCurrency(burn_down.remaining)}</strong></span>
                        </div>
                    </CardContent>
                </Card>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <Card className="hover-zoom">
                    <CardHeader>
                        ...
                        <p className="text-sm text-muted-foreground">Produtos que mais impactaram o resultado.</p>
                    </CardHeader>
                    <CardContent>
                        {top_movers.sku.length > 0 ? (
                            <div className="space-y-3">
                                {top_movers.sku.map((item, i) => (
                                    <div key={i} className="flex items-center justify-between p-3 bg-card-alt/50 rounded-lg border border-border/50">
                                        <span className="font-medium text-sm truncate mr-2 text-foreground" title={item.sku}>{item.sku}</span>
                                        <span className={`font-bold text-sm ${item.delta_lucro >= 0 ? 'text-accent-green' : 'text-accent-red'}`}>
                            ...
                            <div className="text-center py-8 text-muted-foreground text-sm">Sem dados suficientes.</div>
                        )}
                    </CardContent>
                 </Card>
                  <Card className="hover-zoom opacity-75">
                     <CardHeader>
                        ...
                        <p className="text-sm text-muted-foreground">Performance das origens de tráfego.</p>
                    </CardHeader>
                    <CardContent className="flex items-center justify-center min-h-[150px]">
                         <div className="text-center px-6 py-4 text-muted-foreground text-sm bg-card-alt/30 rounded-lg border border-dashed border-border">
...
const KPI: React.FC<{ title: string; value: string; delta?: number, icon?: React.ReactNode }> = ({ title, value, delta, icon }) => (
  <Card className="shadow-sm hover-zoom">
    <CardHeader className="pb-2">
      ...
      <div className="text-2xl font-bold tracking-tight text-foreground mt-2">{value}</div>
    </CardHeader>
    <CardContent className="pt-0 pb-5">
      {typeof delta === 'number' && (
        <div className="text-xs flex items-center gap-1.5 font-medium mt-1">
          {delta >= 0 ? <TrendingUpIcon className="h-3.5 w-3.5 text-accent-green" /> : <TrendingDownIcon className="h-3.5 w-3.5 text-accent-red" />}
          <span className={delta >= 0 ? "text-accent-green" : "text-accent-red"}>{Math.abs(delta).toFixed(1)}%</span>
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            <Card className="shadow-sm hover-zoom">
                <CardHeader>
                    ...
                    <p className="text-sm text-muted-foreground">Conversão de tráfego em receita.</p>
                </CardHeader>
                <CardContent>
                     <FunnelChart data={[
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="bg-card p-6 rounded-xl border border-border hover-zoom flex items-center justify-between shadow-sm">
                    <div>
                        <h3 className="font-bold text-foreground mb-4 flex items-center space-x-2">
                            ...
                            <span>Cartão <span className="text-muted-foreground font-normal text-xs ml-1">({periodLabel})</span></span>
                        </h3>
                        <div className="space-y-1.5 text-sm">
                            <p className="text-muted-foreground flex justify-between gap-6"><span>Aprovados:</span><span className="font-semibold text-foreground">{formatNumber(approval.cartao.approved)}</span></p>
                            <p className="text-muted-foreground flex justify-between gap-6"><span>Reprovados:</span><span className="font-medium text-accent-red">{formatNumber(approval.cartao.rejected)}</span></p>
                        </div>
                    </div>
                 ...
                 <div className="bg-card p-6 rounded-xl border border-border hover-zoom flex items-center justify-between shadow-sm">
                    <div>
                        <h3 className="font-bold text-foreground mb-4 flex items-center space-x-2">
                            ...
                            <span>Pix <span className="text-muted-foreground font-normal text-xs ml-1">({periodLabel})</span></span>
                        </h3>
                        <div className="space-y-1.5 text-sm">
                            <p className="text-muted-foreground flex justify-between gap-6"><span>Aprovados:</span><span className="font-semibold text-foreground">{formatNumber(approval.pix.approved)}</span></p>
                            <p className="text-muted-foreground flex justify-between gap-6"><span>Pendentes:</span><span className="font-medium text-accent-orange">{formatNumber(approval.pix.pending)}</span></p>
                        </div>
                    </div>
                 ...
                 <div className="bg-card p-6 rounded-xl border border-border hover-zoom flex items-center justify-between shadow-sm">
                    <div>
                        <h3 className="font-bold text-foreground mb-4 flex items-center space-x-2">
                            ...
                            <span>Boleto <span className="text-muted-foreground font-normal text-xs ml-1">({periodLabel})</span></span>
                        </h3>
                        <div className="space-y-1.5 text-sm">
                            <p className="text-muted-foreground flex justify-between gap-6"><span>Aprovados:</span><span className="font-semibold text-foreground">{formatNumber(approval.boleto.approved)}</span></p>
                             <p className="text-muted-foreground flex justify-between gap-6"><span>Pendentes:</span><span className="font-medium text-accent-orange">{formatNumber(approval.boleto.pending)}</span></p>
                        </div>
                    </div>
            ...
            <Card className="shadow-sm">
                <CardContent className="p-4 flex items-center gap-3 text-sm">
                    <span className="font-medium text-muted-foreground">Status da Reconciliação:</span>
             ...
             <Card className="shadow-sm">
                <CardHeader className="flex flex-row items-center justify-between pb-4">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-1">{report.ui.subtitle}</p>
                    </div>
                    <div className="flex gap-2">
                         <button className="flex items-center gap-2 px-3 py-2 text-xs font-semibold border border-border rounded-lg hover:bg-card-alt transition-colors text-foreground">
                ...
                <CardContent className="p-0">
                     <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead className="bg-card-alt/50 border-y border-border">
                                <tr>
                                    <th className="px-6 py-3 text-left font-medium text-muted-foreground uppercase tracking-wider">KPI</th>
                                    <th className="px-6 py-3 text-left font-medium text-muted-foreground uppercase tracking-wider">Período</th>
                                    <th className="px-6 py-3 text-right font-medium text-muted-foreground uppercase tracking-wider">Meta</th>
                                    <th className="px-6 py-3 text-right font-medium text-muted-foreground uppercase tracking-wider">Realizado</th>
                                    <th className="px-6 py-3 text-right font-medium text-muted-foreground uppercase tracking-wider">Delta (Δ)</th>
                                    <th className="px-6 py-3 text-center font-medium text-muted-foreground uppercase tracking-wider">Status</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-border bg-card">
                                {report.table.map((row, i) => (
                                    <tr key={i} className="hover:bg-card-alt/30 transition-colors">
                                        <td className="px-6 py-4 font-medium text-foreground">{row.kpi}</td>
                                        <td className="px-6 py-4 text-muted-foreground">{row.period}</td>
                                        <td className="px-6 py-4 text-right tabular-nums text-muted-foreground">{row.plan ? formatCurrency(row.plan) : '—'}</td>
                                        <td className="px-6 py-4 text-right tabular-nums font-bold text-foreground">{formatCurrency(row.actual)}</td>
                                        <td className={`px-6 py-4 text-right tabular-nums font-medium ${(row.delta || 0) >= 0 ? 'text-accent-green' : 'text-accent-red'}`}>
    ...
    if (!reports || !reports.financeReport) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-4 animate-pulse">
                <RefreshIcon className="w-8 h-8 text-muted-foreground/50 animate-spin" />
                <p className="text-muted-foreground font-medium">Gerando relatórios...</p>
            </div>
        );
    }
    ...
    return (
        <div className="bg-background animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                    <h1 className="text-3xl font-bold text-foreground tracking-tight">Relatórios Avançados</h1>
                    <p className="text-muted-foreground mt-1">Análises detalhadas de performance e saúde financeira.</p>
                </div>
                <div className="flex items-center gap-3 self-start md:self-auto">
                     <Badge variant="outline" className="py-1.5 px-3 text-xs font-medium gap-1.5 text-muted-foreground">

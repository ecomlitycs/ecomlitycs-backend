...
const ProductCosts: React.FC<ProductCostsProps> = ({ products, onCostChange, onViewDetails, showToast }) => {
    ...
    const getMarkupColor = (markup: number | null) => {
        if (markup === null || !isFinite(markup)) return 'text-muted-foreground';
        if (markup < 2.5) return 'text-accent-red';
        if (markup < 3.5) return 'text-accent-orange';
    ...
    return (
        <div className="bg-card p-6 rounded-xl border border-border hover-zoom">
            <div className="flex flex-wrap items-center justify-between mb-6 gap-4 border-b border-border pb-6">
                <div>
                    <h3 className="text-lg font-bold text-foreground">Gestão de Custos de Produtos</h3>
                    <p className="text-sm text-muted-foreground mt-1">As alterações salvas são sincronizadas automaticamente com sua loja Shopify.</p>
                </div>
                <button
                    onClick={handleFullSync}
                    disabled={isFullSyncing || syncingProductId !== null}
                    className="flex items-center justify-center space-x-2 bg-white text-primary border-2 border-primary font-semibold px-4 py-2 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed hover:bg-primary/10"
                >
            ...
            {productsWithoutCost.length > 0 && (
                <div className="bg-accent-orange/10 border-l-4 border-accent-orange text-accent-orange p-4 rounded-r-lg mb-6 flex items-start space-x-3">
                ...
                <table className="w-full min-w-[800px]">
                    <thead>
                        <tr className="border-b-2 border-border">
                            <th className="p-3 text-left text-xs font-semibold uppercase text-muted-foreground tracking-wider">Produto</th>
                            <th className="p-3 text-right text-xs font-semibold uppercase text-muted-foreground tracking-wider">Custo de Produto</th>
                            <th className="p-3 text-right text-xs font-semibold uppercase text-muted-foreground tracking-wider">Preço de Venda</th>
                            <th className="p-3 text-right text-xs font-semibold uppercase text-muted-foreground tracking-wider">% C. PRODUTO</th>
                            <th className="p-3 text-right text-xs font-semibold uppercase text-muted-foreground tracking-wider">Markup</th>
                            <th className="p-3 text-center text-xs font-semibold uppercase text-muted-foreground tracking-wider">Ações</th>
                        </tr>
                    </thead>
                    <tbody>
                        {sortedProducts.map(product => {
                            ...
                            return (
                                <tr key={product.id} className={`border-b border-border last:border-b-0 ${!hasCost && !isEditing ? 'bg-accent-red/5' : ''}`}>
                                    <td className="p-3">
                                        <div className="flex items-center space-x-4">
                                            <img src={product.imageUrl} alt={product.name} className="w-12 h-12 rounded-md object-cover bg-border" />
                                            <div>
                                                <p className="font-semibold text-foreground text-sm">{product.name}</p>
                                                {!hasCost && !isEditing && <p className="text-xs text-accent-red font-medium">Custo pendente</p>}
                                            </div>
                                        </div>
                                    </td>
                                    <td className="p-3 text-right w-48">
                                        {isEditing ? (
                                            <div className="relative">
                                                <span className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-2 text-muted-foreground text-sm">R$</span>
                                                <input
                                                    type="text"
                                                    value={tempCost}
                                                    onChange={handleCostInputChange}
                                                    className="w-full rounded-md border-primary bg-card shadow-sm pl-8 pr-2 py-1 text-right font-semibold text-foreground ring-2 ring-primary outline-none text-sm"
                                                    placeholder="0,00"
                                                    autoFocus
                                                    disabled={isSyncing}
                                                />
                                        ...
                                        <div className="flex items-center justify-center space-x-2">
                                            {isEditing ? (
                                                <>
                                                    ...
                                                    <button onClick={handleCancelClick} className="px-3 py-1 text-sm font-semibold rounded-md bg-card-alt hover:bg-border disabled:opacity-70" disabled={isSyncing}>
                                                        Cancelar
                                                    </button>
                                                </>
                                            ) : (
                                                <>
                                                    <button onClick={() => handleEditClick(product)} className="px-3 py-1 text-sm font-semibold rounded-md bg-card-alt hover:bg-border">Editar</button>
                                                    <button onClick={() => onViewDetails(product)} className="px-3 py-1 text-sm font-semibold rounded-md text-primary hover:bg-primary-soft">Detalhes</button>
                                                </>
                                            )}
                                        </div>
                                    </td>

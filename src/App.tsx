...
import PlanningDashboard from './src/components/PlanningDashboard';
import AnnualPlanningDashboard from './src/components/AnnualPlanningDashboard';
import PricingCalculator from './src/components/PricingCalculator';
import TacticalTracking from './src/components/TacticalTracking';
...
import { MenuIcon, CheckCircleIcon, RefreshIcon } from './src/components/icons';
...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    switch(activeTab) {
      ...
      case 'data': return <Integrations integrations={integrations} handleIntegrationToggle={() => {}} />;
  ...
  return (
    <div className="flex h-screen bg-background dark:bg-dark-background text-foreground dark:text-dark-foreground antialiased font-sans">
      <Sidebar 
        activeTab={activeTab} 
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 flex-shrink-0 bg-card/80 dark:bg-dark-card/95 backdrop-blur-sm border-b border-border dark:border-dark-border flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4">
             <button onClick={() => setIsSidebarOpen(true)} className="text-muted-foreground hover:text-foreground dark:text-dark-muted-foreground dark:hover:text-dark-foreground lg:hidden">
                <MenuIcon className="w-6 h-6" />
            </button>
             <h1 className="text-lg font-headings font-bold text-foreground dark:text-dark-foreground">{headerTitleMap[activeTab]}</h1>
             <div className="flex items-center text-xs text-muted-foreground dark:text-dark-muted-foreground border-l border-border dark:border-dark-border ml-4 pl-4 hidden sm:flex space-x-3">
                <div className="flex items-center space-x-1.5 bg-card-alt dark:bg-dark-card-alt px-2 py-1 rounded-full">
                    <span className="relative flex h-2 w-2">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-dark-accent-green opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-2 w-2 bg-dark-accent-green"></span>
                    </span>
                    <span className="font-medium text-foreground dark:text-dark-foreground">Dados atualizados há 37 min</span>
                </div>
                <button className="flex items-center gap-1 text-primary hover:text-primary/80 dark:text-dark-primary-soft-fg dark:hover:text-dark-primary-soft-fg/80 transition-colors font-medium">
